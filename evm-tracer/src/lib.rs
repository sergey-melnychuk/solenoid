...
/*
TODO: Trace COINBASE BALANCE on every opcode
...
struct CoinbaseTracer {
    coinbase: Address,
    trigger: bool,
}

impl<CTX> Inspector<CTX> for CoinbaseTracer {
    fn step(&mut self, interp: &mut Interpreter, context: &mut CTX) {
        self.trigger = interp.current_opcode() == COINBASE;
        if interp.current_opcode() == BALANCE {
            let addr = Address::from(interp.stack().peek(0).unwrap());
        }
    }
    
    fn step_end(&mut self, interp: &mut Interpreter, context: &mut CTX) {
        if interp.current_opcode() == COINBASE {
            let addr = Address::from(interp.stack().peek(0).unwrap());
            self.coinbase = addr;
        }
        if interp.current_opcode() == BALANCE && self.trigger {
            let addr = Address::from(interp.stack().peek(0).unwrap());
            if addr == self.coinbase {
                let balance = interp.stack().peek(0).unwrap();
                eprintln!("COINBASE BALANCE: {:#x}", balance);
            }
        }
    }

    fn call_end(&mut self, context: &mut CTX, inputs: &CallInputs, outcome: &mut CallOutcome) {
        // Check coinbase balance after each call via journal
        if let Ok(balance) = context.journal().balance(self.coinbase) {
            eprintln!("Coinbase balance after call: {:#x}", balance.data);
        }
    }
}
